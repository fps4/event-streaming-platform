...
import { renderMermaidBlocks } from "../src/lib/mermaid.js";
...
async function translateText(text, targetLang, sourceLang = "en") {
    const prompt = `
        ...
        Don't enclose the resulting content into codeblock. 
        Ensure title and subtitle is always translated. 
        Ensure lables in pagelinks, tiles, ctaButton translated as well.
        Don't translate any links.
        Keep the frontmatter and formatting intact:\n\n${text}`;
...
function slugSegmentsFromRelPath(relPath) {
    return relPath.split(path.sep);
}

async function ensureMermaidAssets({ lang, relPath, markdown }) {
    if (!markdown.includes("```mermaid")) {
        return;
    }

    const { content } = matter(markdown);
    const slugSegments = slugSegmentsFromRelPath(relPath);

    try {
        await renderMermaidBlocks(content, { lang, slugSegments });
        console.log(`   ↳ Mermaid assets refreshed for ${lang}/${relPath}`);
    } catch (error) {
        console.warn(`   ⚠️ Mermaid render skipped for ${lang}/${relPath}:`, error.message || error);
    }
}
...
async function processFiles(fileName = null) {
    ...
    if (fileName) {
        ...
        files = glob.sync(`${CONTENT_DIR}/${SOURCE_LANG}/**/*.md`); // Updated to include files in root and subfolders
    }
    ...
    for (const file of files) {
        const relPath = path.relative(`${CONTENT_DIR}/${SOURCE_LANG}`, file); // e.g. services/ai-enablement.md
        ...
        await ensureMermaidAssets({ lang: SOURCE_LANG, relPath, markdown: content });
        ...
        for (const lang of TARGET_LANGS) {
            ...
            try {
                ...
                await ensureMermaidAssets({ lang, relPath, markdown: translated });
            } catch (err) {
